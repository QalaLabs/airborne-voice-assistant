"""
Local last-resort knowledge fallback for the ADK agent's search_knowledge
tool, used only if the call to Cloud Run's /internal/rag/query endpoint
fails (see agent/rag_client.py). This is a copy of the static
AIRBORNE_KNOWLEDGE_BASE + search_local_knowledge() from rag.py (repo root,
Cloud Run side) -- duplicated deliberately so the agent's deploy artifact
doesn't need psycopg2/OpenAI/database.py, at the accepted cost of the two
copies needing to be kept in sync manually if the knowledge base changes.
Cloud Run's rag.py + Postgres/pgvector remains the source of truth; this is
a fallback of a fallback.
"""
...
def search_local_knowledge(query: str, top_k: int = 3) -> str:
    """
    Fast token-based keyword relevance scoring, identical logic to
    rag.py's search_local_knowledge -- kept as a pure-Python, dependency-free
    fallback for use inside the Agent Engine deploy artifact.
    """
